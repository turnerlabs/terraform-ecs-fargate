...
EOF
  ...
  vars = {
    ...
    sidecar_revision       = "${element(split(":", aws_ecs_task_definition.secrets_sidecar.arn), 6)}"
...
output "deploy_secrets_sidecar" {
  value = "fargate service deploy --revision ${element(split(":", aws_ecs_task_definition.secrets_sidecar.arn), 6)}"
}
